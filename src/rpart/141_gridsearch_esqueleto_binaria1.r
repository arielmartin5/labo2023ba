...
PARAM$semillas <- c(101666, 166666, 196666, 106666, 166666)
...
particionar <- function(data, division, agrupa = "", campo = "fold", start = 1, seed = NA) {
  ...
  dataset[, (campo) := sample(rep(bloque, ceiling(.N / length(bloque))))[1:.N],
    by = agrupa
  ]
}
#------------------------------------------------------------------------------
...
# Creeamos el campo clase_binaria1 en el dataset
library(dplyr)
dataset <- mutate(dataset, clase_binaria1 = ifelse(clase_ternaria == "BAJA+2", "pos", "neg"))
...
archivo_salida <- "./exp/HT2020/gridsearch__binaria1_001.txt"
...
for (vcp in c(-1, -0.5)) {
    for (vmax_depth in c(7, 8, 10, 12, 14, 16)) {
        for (vmin_split in c(1200, 1000, 800, 600, 500, 400)) {
            for (vmin_bucket in c(8, 16, 100, 250, 440)) {
